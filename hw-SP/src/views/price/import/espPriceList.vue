  ...
  <div>
    ...
    <el-row>
      ...
      <el-col :span="8" class="text-right">
        ...
        <el-button type="primary" @click="goAdjustList()">加价或优惠调整</el-button>
      </el-col>
    </el-row>
    <el-table
      ...
      <el-table-column align="center" label="物理指标查看" min-width="60">
        <template slot-scope="scope">
          <i class="text-center el-icon-search" style="cursor: pointer">
          </i>
        </template>
      </el-table-column>
      <el-table-column align="center" label="历史价格查询" min-width="60">
        <template slot-scope="scope" >
          <i class="text-center el-icon-search" style="cursor: pointer" @click="goHistoryList(scope.row)">
          </i>
        </template>
      </el-table-column>
    </el-table>
    ...
    methods: {
      ...
      goAdjustList() {
        if (this.multipleSelection.length > 0) {
          let specialPriceIds = this.multipleSelection.map(item => item.fdId).join(';')
      ...
      goHistoryList(item) {
        this.$router.push({
          path: '/price/import/espPriceListOld/' + item.batchId,
        })
      },
